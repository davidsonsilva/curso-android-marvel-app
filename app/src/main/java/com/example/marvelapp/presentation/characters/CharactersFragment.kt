...
class CharactersFragment : Fragment(), SearchView.OnQueryTextListener,
    ...
    private val charactersAdapter: CharactersAdapter by lazy {
        CharactersAdapter(imageLoader = imageLoader) { character, view ->
            ...
            findNavController().navigate(directions = directions, extras)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val menuHostFragment: MenuHost = requireActivity()
        menuHostFragment.addMenuProvider(object : MenuProvider {
            override fun onCreateMenu(menu: Menu, menuInflater: MenuInflater) {
                ...
                if(viewModel.currentSearchQuery.isNullOrEmpty().not()){
                ...
                return when (menuItem.itemId) {
                    R.id.menu_sort -> {
                        findNavController().navigate(
                            R.id.action_charactersFragment_to_sortFragment
                        )
                        true
                    }
                    else -> false
    ...
    private fun observeSortingData() {
        ...
        val observer = LifecycleEventObserver { _, event ->
            val isSortingApplied = navBackStackEntry.savedStateHandle
                .contains(SORTING_APPLIED_BASK_STACK_KEY)
